...
<%
    // DEBUG: First check all attributes
    System.out.println("=== DEBUG addMoodDetails.jsp ===");
    
    // Check for date in model attribute (from controller)
    String dateFromAttribute = (String) request.getAttribute("date");
    System.out.println("1. Date from request attribute (model): " + dateFromAttribute);
    
    // Check for date in request parameter
    String dateFromParam = request.getParameter("date");
    System.out.println("2. Date from request parameter: " + dateFromParam);
    
    // Get mood entry for editing
    MoodEntry moodToEdit = (MoodEntry) request.getAttribute("moodToEdit");
    boolean isEdit = moodToEdit != null;
    
    String dateFromMoodEntry = null;
    if (isEdit && moodToEdit != null) {
        dateFromMoodEntry = moodToEdit.getEntryDate().toString();
        System.out.println("3. Date from mood entry: " + dateFromMoodEntry);
    }
    ...
    // Use the date from attribute first (from controller)
    String dateParam = dateFromAttribute;
    if (dateParam == null || dateParam.isEmpty()) {
        dateParam = dateFromParam;
    }
    // FIX: In edit mode, fall back to date from mood entry
    if (isEdit && (dateParam == null || dateParam.isEmpty())) {
        dateParam = dateFromMoodEntry;
        System.out.println("DEBUG: Using date from mood entry in edit mode: " + dateParam);
    }
    
    System.out.println("Final date value: " + dateParam);
    ...
    // Build back URL with selected feelings - FIX for edit mode
    StringBuilder backUrl = new StringBuilder(request.getContextPath() + "/mood?action=" + (isEdit ? "edit" : "add"));
    
    // Add date parameter if exists
    if (dateParam != null && !dateParam.isEmpty()) {
        backUrl.append("&date=").append(dateParam);
    }
    
    // Add feelings parameters
...
<div class="container">
    ...
    <form action="<%= request.getContextPath() %>/mood" method="post" enctype="multipart/form-data">
        ...
        <% if (dateParam != null && !dateParam.isEmpty()) { %>
            ...
            <!-- <div style="color: green; font-size: 12px; margin-bottom: 10px;">
                DEBUG: Passing date to save: <%= dateParam %> 
                <%= isEdit ? "(Edit mode)" : "(Add mode)" %>
            </div> -->
        <% } else { %>
            <!-- <div style="color: red; font-size: 12px; margin-bottom: 10px;">
                DEBUG: WARNING: No date parameter found!
            </div> -->
        <% } %>
